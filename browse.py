...
add_files_list = [] # new added files storage queue
	...
	def keypress(self, (maxcol,), key):
		...
		if key == " " or key == "enter":
			...
			if os.path.isdir(path):
				last_visited_dir = path
				for a in os.listdir(path):
					if os.path.isfile(path+"/"+a) and os.path.splitext(a)[1] in ALLOWED_FORMATS: add_files_list.append((path+"/"+a, a))
			else:
				last_visited_dir = self.dir
				add_files_list.append((path, self.name))
		else:
			return key
		...
		try:
			...
			for a in os.listdir(path):
				if os.path.isdir(os.path.join(path,a)):
					...
					if os.path.splitext(a)[1].lower() in ALLOWED_FORMATS:
						files.append(a)
		except OSError, e:
...
class DirectoryBrowser:
	palette = [
		...
		('path_edit_title', 'white', 'black', 'standout')
		]
	...
	def __init__(self, cwd=None):
		global last_visited_dir
		if not cwd or not os.path.isdir(cwd): cwd = os.getcwd()
		last_visited_dir = cwd
		...
		self.add_edit = urwid.Edit(('path_edit_title', "Path: "),  wrap='clip')
		self.last_size = (0, 0)
		...
		if self.pile_in_focus == 0:
			for k in keys:
				if k == 'right':
					# right can behave like +
					k = "+"
				if k != 'backspace': k = self.view.keypress( size, k )
				# k = unhandled key or None
				if k == 'left':
					self.move_focus_to_parent( size )
				elif k == '-':
					...
					if self.move_focus_to_parent( size ):
						self.view.keypress( size, 'enter' )
					
		else:
			if 'enter' in keys:
				path = self.add_edit.get_edit_text()
				if os.path.isdir(path):
					for a in os.listdir(path):
						if os.path.isfile(path+"/"+a) and os.path.splitext(a)[1] in ALLOWED_FORMATS: add_files_list.append((path+"/"+a, a))
				elif os.path.isfile(path):
					add_files_list.append((path, os.path.basename(path)))
				elif path.startswith("http://"):
					add_files_list.append((path, path))
			elif 'end' in keys:
				self.add_edit.set_edit_text("")
			else:				
				for key in keys: self.add_edit.keypress((size[0],), key)
	...
	def __setup_pile(self,  size):
		...
		self.pile = urwid.Pile([
		                   urwid.BoxAdapter(self.view,  size[1]-1),  
						   self.add_edit
						   ])
